...
test('Register() must have a name and size', (t) => {
  ...
  t.is(err.message, 'Register(name, size) must be defined');
  new t.context.Register('REG', 4);
  t.pass();
});

test('bit() should return 0b1111', (t) => {
  let reg = new t.context.Register('REG', 4, 0b1111);
  t.is(reg.bit(0), 1);
  t.is(reg.bit(1), 1);
  t.is(reg.bit(2), 1);
  t.is(reg.bit(3), 1);
...
test('bit() should return 0b1000', (t) => {
  let reg = new t.context.Register('REG', 4, 0b1000);
  t.is(reg.bit(0), 0);
  t.is(reg.bit(1), 0);
  t.is(reg.bit(2), 0);
  t.is(reg.bit(3), 1);
...
test('bitSet() should be chainable', (t) => {
  let reg = new t.context.Register('REG', 4, 0b0000);
...
test('load() should be chainable', (t) => {
  let reg1 = new t.context.Register('REG', 4, 0b0000);
  let reg2 = new t.context.Register('REG', 4, 0b1111);
...
test('wrap() should fix underflown Registers', (t) => {
  let reg = new t.context.Register('REG', 4, 0b0);
  reg.bits -= 1; // overflow
  t.is(reg.bits, -1);
  reg.wrap();
...
test('toString() new Register', (t) => {
  let reg = new t.context.Register('REG', 1);
  t.is(reg.toString(), '[object Register 0b0 REG]');
});
